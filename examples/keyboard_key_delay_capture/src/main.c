...
/**
  * @brief  pb.13 gpio configuration.
  * @param  none
  * @retval none
  */
...
/**
  ...
  * @param  PB13 输出推挽电平控制被测试键盘按键on/off
  * @retval none
  */
void EXINT0_IRQHandler(void)
{
  ...
  if(USBH_CLASS == otg_core_struct.host.global_state)
  {
    ...
    USBH_DEBUG("Start testing key delay time\n");
    GPIOB->clr = GPIO_PINS_13; 
    delay_ms(50);
    /* set pb.13 */
    GPIOB->scr = GPIO_PINS_13;      //按键按下
    tmr_counter_enable(TMR2, TRUE); //开始计时
  }
   else
  {
    USBH_DEBUG("Please Wakeup Keyboard ! ! !\n"); 
    delay_ms(50); 
  }
}
...
int main(void)
{
  ...
  tmr_base_init(TMR2, 0xFFFFFFFF, (crm_clocks_freq_struct.sclk_freq / 1000000) - 1);
  ...
  /*操作提示*/
  USBH_DEBUG("READY TO KEY DELAY TIME CAPTURE\n ");
