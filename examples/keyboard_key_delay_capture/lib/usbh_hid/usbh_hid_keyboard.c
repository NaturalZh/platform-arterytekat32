...
void usbh_hid_keyboard_decode(uint8_t *data)
{
  ...
  uint32_t key_receive;  //收到按键消息的时间
  ...
  if(nb_keys_n == 1)
  {
    key_receive = tmr_counter_value_get(TMR2); //获取按键延时值
    key_newest = keys_n[0];
    ...
    USBH_DEBUG("Capture key is [%c]", out);
    if(key_receive < 50000)
    {
      if(key_receive < 10)
      {
        USBH_DEBUG("Please Don`t push-pull the keyboard switch directly !!!");
      }
      else
      {
        USBH_DEBUG("The Key received delay time is [%u] uS\n", key_receive); //打印按键延时
      }
    }
    else
    { 
      USBH_DEBUG("The key delay time is more than 50 mS !!!");
    }
